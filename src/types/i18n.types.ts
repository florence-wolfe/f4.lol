...
type Equal<X, Y> = (<T>() => T extends X ? 1 : 2) extends <T>() => T extends Y ? 1 : 2 ? true : X;

type Exact<T, U> = T extends U ? (U extends T ? T : never) : never;

// Define a utility type to compare the types of imported locales
type CompareLocales<T extends readonly unknown[]> = T extends [infer First, ...infer Rest]
...
export type AreLocalesValid<T extends unknown[]> = Equal<CompareLocales<T>, true>;
